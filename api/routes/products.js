...
router.post('/', authenticateShop, async (req, res) => {
  try {
    const { name, description, price, originalPrice, stockQuantity, categoryId, imageUrl } = req.body;
    ...
    const productId = `${name.toLowerCase().replace(/\s+/g, '-')}-${Date.now()}`;
...
router.put('/:productId', authenticateShop, async (req, res) => {
  try {
    const { productId } = req.params;
    const { name, description, price, originalPrice, stockQuantity, categoryId, imageUrl, isActive } = req.body;
