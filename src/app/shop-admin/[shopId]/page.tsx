...
export default function ShopAdminPage({ params }: { params: Promise<{ shopId: string }> }) {
  ...
  const handleProductSubmit = async () => {
    ...
    try {
      ...
      let response;
      if (editingProduct) {
        response = await fetch(`/api/shop-products/${editingProduct.product_id}`, {
          method: 'PUT',
          headers: {
  ...
  const handleCategorySubmit = async () => {
    ...
    try {
      ...
      let response;
      if (editingCategory) {
        response = await fetch(`/api/shop-products/categories/${editingCategory.id}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`,
  ...
  const handleToggleProductStatus = async (productId: string) => {
    const confirm = window.confirm('Are you sure you want to Deactivate the product?');
    if (!confirm) return; 
    setLoading(true);
    try {
      const response = await fetch(`/api/shop-products/${productId}/toggle-status`, {
        method: 'PATCH',
        headers: { 'Authorization': `Bearer ${token}` },
      });
  ...
  const handleDeleteCategory = async (categoryId: number) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/shop-products/categories/${categoryId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` },
      });
      ...
      {viewMode === 'grid' ? (
        <Box display="flex" flexWrap="wrap" gap={3}>
          {sortedProducts.map((product) => (
            <Box sx={{ flex: '1 1 300px', maxWidth: '400px', minWidth: '280px' }} key={product.id}>
              <Card 
                sx={{ 
                  height: '100%',     
                  display: 'flex', 
              ...
              >
                ...
                <CardContent sx={{ flexGrow: 1, p: 2 }}>
                  ...
                  <Box display="flex" justifyContent="space-between" alignItems="flex-start" mb={1}>
                    <Typography variant="h6" noWrap sx={{ maxWidth: '70%', fontWeight: 'bold' }}>
                      {product.name}
                    </Typography>
                    <Box>
                      <IconButton 
                      ...
                      <Button 
                        size="small" 
                        variant="outlined"
                        onClick={() => handleToggleProductStatus(product.product_id)}
                        sx={{ 
                          color: product.is_active ? 'warning.main' : 'success.main',
                          borderColor: product.is_active ? 'warning.main' : 'success.main',
                          '&:hover': {
                            borderColor: product.is_active ? 'warning.dark' : 'success.dark',
                            backgroundColor: product.is_active ? 'warning.light' : 'success.light'
                          }
                        }}
                      >
                  ...
                  <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                    ...
                    <Box display="flex" alignItems="center">
                      ...
                      <Typography variant="body2" color={product.stock_quantity > 0 ? 'success.main' : 'error.main'}>
                  ...
                  <Box display="flex" justifyContent="space-between" alignItems="center" mt={2}>
                    ...
                    <Button
                      ...
                      variant="outlined"
                      startIcon={<Share />}
                      onClick={() => window.open(`/website/${shopId}`, '_blank')}
                      fullWidth
                    >
                      View Shop
                    </Button>
                  ...
                  <TableRow key={product.id}>
                    ...
                    <TableCell>
                      <Box display="flex" gap={1}>
                        <IconButton size="small" onClick={() => openProductPreview(product)}>
                          <Visibility />
                        </IconButton>
                        ...
                        <IconButton size="small" onClick={() => handleDeleteProduct(product.product_id)}>
                          <Delete />
                        </IconButton>
                      </Box>
                    </TableCell>
    ...
    <Box sx={{ display: 'flex', minHeight: '100vh', bgcolor: '#f5f5f5' }}>
      ...
      <Dialog open={productDialogOpen} onClose={() => setProductDialogOpen(false)} maxWidth="md" fullWidth>
        ...
        <DialogActions sx={{ p: 3 }}>
          <Button onClick={() => setProductDialogOpen(false)} variant="outlined">
            Cancel
          </Button>
      ...
      <Dialog open={previewDialogOpen} onClose={closeProductPreview} maxWidth="lg" fullWidth>
        <DialogTitle>
          <Box display="flex" justifyContent="space-between" alignItems="center">
      ...
      <Dialog open={categoryDialogOpen} onClose={() => setCategoryDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogActions sx={{ p: 3 }}>
          <Button onClick={() => setCategoryDialogOpen(false)} variant="outlined" disabled={loading}>
            Cancel
          </Button>
          <Button 
          ...
          <Box display="flex" justifyContent="space-between" alignItems="center">
            <Typography variant="h6">
              ...
              {unreadNotificationsCount > 0 && (
                <Badge badgeContent={unreadNotificationsCount} color="error" sx={{ ml: 2 }}>
                  <Notifications />
                </Badge>
              )}
            </Typography>
        ...
        <DialogContent>
          {notifications.length === 0 ? (
            <Box textAlign="center" py={4}>
              ...
              <Typography variant="body2" color="text.secondary">
                You'll see notifications here when you receive new orders or updates.
              </Typography>
            </Box>
          ) : (
            <List>
          ...
          {selectedOrder && (
            <Box>
              ...
              <Box mb={3}>
                ...
                <Typography variant="body2">
                  <strong>Email:</strong> {selectedOrder.user?.email || 'No email'}
                </Typography>
                <Typography variant="body2">
                  <strong>Phone:</strong> {selectedOrder.user?.phone || 'No phone'}
                </Typography>
              </Box>
