...
export default function WebsitePage({ params }: { params: Promise<{ websiteId: string }> }) {
  ...
  useEffect(() => {
    const fetchWebsiteData = async () => {
      try {
        ...
        try {
          const shopCheckResponse = await fetch(`/api/shops/check/${data.website.phone}`);
          if (shopCheckResponse.ok) {
            ...
            if (productsResponse.ok) {
              ...
              console.log('Products with categories:', productsData.products?.map((p: any) => ({ 
                name: p.name, 
            ...
            if (categoriesResponse.ok) {
              ...
              console.log('Number of categories:', categoriesData.categories?.length || 0);
              ...
              if (productsData && productsData.products) {
                const categoriesWithCount = categoriesData.categories.map((cat: any) => {
                  const productCount = productsData.products.filter((product: Product) => 
                ...
                const productsWithoutCategory = productsData.products.filter((product: Product) => 
                  !product.category_id
                ).length;
                ...
                console.log('Categories without count:', categoriesData.categories);
  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: '#f8f9fa' }}>
      ...
      {/* <Container sx={{ py: 0 }}> */}
        ...
        {/* Products Grid */}
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, 1fr)', md: 'repeat(3, 1fr)', lg: 'repeat(4, 1fr)' }, gap: 3,px:4 }}>
              ...
              <Card sx={{
                ...
                <CardActions sx={{ p: 2, pt: 0 }}>
                  ...
                  <IconButton
                    ...
                    color={wishlist.includes(product.product_id) ? 'error' : 'default'}
                  >
                    {wishlist.includes(product.product_id) ? <Favorite /> : <FavoriteBorder />}
                  </IconButton>
                </CardActions>
              </Card>
            </Box>
                ...
                <Box>
                  ...
                  <Box display="flex" gap={2} mb={3}>
                    ...
                    <IconButton
                      ...
                      color={wishlist.includes(selectedProduct.product_id) ? 'error' : 'default'}
                    >
                      {wishlist.includes(selectedProduct.product_id) ? <Favorite /> : <FavoriteBorder />}
                    </IconButton>
                  </Box>
